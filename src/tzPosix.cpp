...
void printPosixData(struct posix_tz_data_struct& posixData, Print& out) {
...
void cleanUpPosixData(struct posix_tz_data_struct& posixData) {
  // trim names
  { // release String as soon as finished with it
    String sTzname = posixData.tzname;
    sTzname.trim();
    strcpy(posixData.tzname, sTzname.c_str()); // will fit since trim() <= length
  }
  { // release String as soon as finished with it
    String sDstTzname = posixData.dsttzname;
    sDstTzname.trim();
    strcpy(posixData.dsttzname, sDstTzname.c_str()); // will fit since trim() <= length
  }
...
void buildPOSIXdescription(struct posix_tz_data_struct & posixData, String & result) {
  ...
  result = ""; // clear result
  if (strcmp(posixData.tzname, "UTC") == 0) {
    result = "UTC";
    return;
  }
